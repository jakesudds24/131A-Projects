...
err(j,1:n_mid(j))     = T - Ta(:,1);
LinfErr(j)   = max(abs(err(j,:)));
L2Err(j)     = norm(err(j,1:n_mid(j)))/sqrt(numel(err(j,1:n_mid(j))));
end

%Plot the least squares error for each number of cells used
figure(2)
hold on
plot(n_mid,L2Err,'ro')
plot(n_mid,L2Err,'r')
hold off
title('Least Squares Error Plotted Against Number of Cells Used')
